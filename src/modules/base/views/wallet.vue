<template>
  ...
  import { createWallet, getAddress } from "../utils/wallet_utils";
	...
	try {
	  ...
	  for (const line of lines) {
		...
		if (name && group && privateKey) {
		  const address = await getAddress(privateKey);  // 解析地址
		  ...
		  if (existingIndex !== -1) {
			...
			// 如果地址不存在，则添加新的数据
			data.value.push({
			  key: idCounter.value.toString(), // 增加唯一键
			  name: name.trim(),
			  group: group.trim(),
			  address: address,
			  privateKey: privateKey.trim(),
  ...
  const handleCreate = async () => {
	try {
	  ...
	  const wallets = await createWallet(group, quantity);
  ...
  const handleDeleteSelected = async () => {
	try {
	  ...
	  await deleteData("WalletManage", addresses); // 执行删除操作
